...
class KNNBasic(SymmetricAlgo):
    ...
    def estimate(self, u, i):
        ...
        if self.yr is None or self.sim is None:
            raise RuntimeError("Algorithm must be fitted before making predictions")
...
class KNNBaseline(SymmetricAlgo):
    ...
    *baseline* rating.
    ...
    def estimate(self, u, i):
        ...
        est = self.trainset.global_mean
        if self.trainset.knows_user(u) and self.bu is not None:
            est += self.bu[u]
        if self.trainset.knows_item(i) and self.bi is not None:
            est += self.bi[i]
        ...
        if self.bx is None or self.by is None:
            raise RuntimeError("Baselines must be computed before making predictions")
        neighbors = [(x2, self.sim[x, x2], r) for (x2, r) in self.yr[y]]
        k_neighbors = heapq.nlargest(self.k, neighbors, key=lambda t: t[1])

        # compute weighted average
        sum_sim = sum_ratings = actual_k = 0
        for (x2, sim, r) in k_neighbors:
            if sim > 0:
                sum_sim += sim
                nb_bsl = self.trainset.global_mean
